...
namespace wci
{
	...
	{
		mSpawnTimer = TimeManager::Get().SetTimer(GetWeakRef(), &ChaosStage::SpawnVanguard, mSpawnInterval);
		mDifficultTimerHandle = TimeManager::Get().SetTimer(GetWeakRef(), &ChaosStage::IncreaseDifficulty, mSpawnIntervalDecrementInterval, true);

		TimeManager::Get().SetTimer(GetWeakRef(), &ChaosStage::StageDurationReached, mStageDuration);
	}

	void ChaosStage::StageFinished()
	{
		TimeManager::Get().ClearTimer(mDifficultTimerHandle);
	...
	void ChaosStage::SpawnVanguard()
	{
		...
		mSpawnTimer = TimeManager::Get().SetTimer(GetWeakRef(), &ChaosStage::SpawnTwinBlade, mSpawnInterval);
	}
	...
	void ChaosStage::SpawnTwinBlade()
	{
		...
		mSpawnTimer = TimeManager::Get().SetTimer(GetWeakRef(), &ChaosStage::SpawnHexagon, mSpawnInterval);
	}
	...
	void ChaosStage::SpawnHexagon()
	{
		...
		mSpawnTimer = TimeManager::Get().SetTimer(GetWeakRef(), &ChaosStage::SpawnUFO, mSpawnInterval);
	}
