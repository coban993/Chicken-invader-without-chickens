...
namespace wci
{
	...
	bool BackgroundLayer::IsSpriteOffScreen(sf::Sprite& sprite) const
	{
		...
		if (spritePos.x < -bound.width || spritePos.x > windowSize.x + bound.width) return true;

		if (spritePos.y < -bound.height || spritePos.y > windowSize.y + bound.height) return true;
